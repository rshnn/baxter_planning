...
// #define PRX_STATUS(stream) std::cout<<"\r"<<stream
#define PRX_STATUS( stream, stream_color )\
{\
    ...
    ( std::cout <<"\r"<< PRX_COLOR____ <<"[PRINT][" << NODE_NAME____ << "][" << FILE_NAME____ << "::" << __FUNCTION__ << "][" << __LINE__ << "]:    " << stream );std::cout.flush();\
}
#define PRX_STATUS_S( stream )\
{\
