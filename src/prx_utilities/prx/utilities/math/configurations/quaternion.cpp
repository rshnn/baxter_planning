...
namespace prx
{
    namespace util
    {
        ...
        double quaternion_t::get_yaw() const
        {
        ...
        {
	    double the_norm = 1.0/norm();
	    double new_q[4];
	    bool is_equal = true;
	    for( int i=0; i<4; i++ )
	    {
		new_q[i] = q[i]*the_norm;
		if( is_equal && fabs( q[i] - new_q[i] ) > PRX_ZERO_CHECK) 
		    is_equal = false;
	    }
	    ...
	    if( lambda < 0.0 )
		for( int i = 0; i< 4; i ++ )
		    q[i] = q[i]*r + target.q[i] *s;
	    else  // i.e., if originally lambda < 0, use the opposite
		for( int i = 0; i< 4; i ++ )
		    q[i] = q[i]*r - target.q[i] *s;
        ...
        {
            return quaternion_t( q.get_w()*v[0]+q.get_y()*v[2]-q.get_z()*v[1],
        ...
        {
            return quaternion_t( q.get_w()*v[0]+q.get_z()*v[1]-q.get_y()*v[2],
