...
namespace prx
{
    ...
    namespace sim
    {
        ...
        void application_t::init(const parameter_reader_t * const reader)
        {
            ...
            has_dynamic_obstacles = reader->get_attribute_as<bool>("application/has_dynamic_obstacles", false);
            ...
            visualization_multiple = ceil(.04/simulation::simulation_step);
...
//                PRX_DEBUG_S("Plant name is: " << iter->first << " : " << iter->second.print());
                tf_broadcaster->queue_config(iter->second, iter->first);
            }
            if(has_dynamic_obstacles)
            {
                index = 0;
                simulator->update_obstacles_configs(config_map, index);
                for( config_list_t::iterator iter = config_map.begin(); iter != config_map.end(); iter++ )
                {
    //                PRX_DEBUG_S("Obstacle name is: " << ros::this_node::getName().substr(1,ros::this_node::getName().length())+"/"+iter->first << " : " << iter->second.print());
                    //        iter->second.print();
                    tf_broadcaster->queue_config(iter->second, ros::this_node::getName().substr(1,ros::this_node::getName().length())+"/"+iter->first);
                }
            }
        ...
        void application_t::update_obstacles_every_frame(bool has_dynamic_obs)
        {
            has_dynamic_obstacles = has_dynamic_obs;
        }
