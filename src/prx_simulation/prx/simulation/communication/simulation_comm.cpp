    ...
    namespace sim
    {
        namespace comm
        {
            ...
            bool simulation_comm_t::request_ground_truth_callback(request_ground_truth_srv::Request& request, request_ground_truth_srv::Response& response)
            {
                ...
                PRX_ASSERT(sim != NULL);
                system_ptr_t smart_sys = sim->get_system(request.consumer_name);
                PRX_DEBUG_S(request.consumer_name);

                consumer_controller_t* controller = dynamic_cast<consumer_controller_t*>(smart_sys.get());

                if( controller == NULL )
                    PRX_ERROR_S("System " << request.consumer_name.c_str() << " is not a consumer_controller_t!");

                //get the state space intervals
                const space_t* consumer_state_space = controller->get_state_space();

                state_t* consumer_point = consumer_state_space->alloc_point();

                consumer_state_space->copy_point_to_vector(consumer_point, response.full_state.elements);
